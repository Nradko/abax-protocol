...
(async () => {
  ...
  for (let i = 0; i <= SAMPLE_SIZE; i++) {
    ...
    //Action
    await lendingPool.withSigner(borrower).query.repay(reserveDAI.address, borrower.address, 1000, [1]);
  ...
  const chartData = getBasicChartConfig(dataSets, 'Deposit interest in time');
