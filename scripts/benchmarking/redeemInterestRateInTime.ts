...
(async () => {
  ...
  const redeemAmountInCurrency = depositAmountInCurrency.divn(daiDepositAmount);
  for (let i = 0; i <= SAMPLE_SIZE; i++) {
    //Move to next data point
    await advanceBlockTimestamp(blockTimestampProvider, STEP);
    //Action
    await lendingPool.withSigner(supplier).query.redeem(reserveDAI.address, supplier.address, redeemAmountInCurrency, []);
    await lendingPool.withSigner(supplier).tx.redeem(reserveDAI.address, supplier.address, redeemAmountInCurrency, []);
  ...
  const chartData = getBasicChartConfig(dataSets, 'Redeem variable interest in time');
