use abax_library::structs::{
    AssetRules, FeeReductions, ReserveFees, ReserveRestrictions,
};
use ink::{
...
pub trait LendingPoolManage {
    ...
    /// sets the fee reduction for the account
    ///
    /// * `account` - account for which fee reduction should be set
    /// * `fee_reductions` - (deposit_fee_reductino_e6, debt_fee_reduction_e6)
    ///
    /// # Errors
    /// * `AccessControl::MisingRole` returned if the caller is not a FEE_REDUCTION_ADMIN.
    /// * `InvalidFeeReduction` returned if the fee reduction is invalid.
    #[ink(message)]
