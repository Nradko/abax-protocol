...
makeSuite.skip('Testing protocol income', () => {
  ...
  for (const feeD6 of feesD6) {
    describe(` fee is set to ${feeD6}, 1 milion of DAI and USDC are supplied by user1 and user2. Then ...`, () => {
      let testEnv: TestEnv;
      ...
      beforeEach('setup Env', async () => {
        ...
        await usdcContract.tx.mint(user1.address, millionUsdc);
        await usdcContract.withSigner(user1).tx.approve(lendingPool.address, millionUsdc);
        await lendingPool.withSigner(user1).tx.deposit(usdcContract.address, user1.address, millionUsdc, []);

        await daiContract.tx.mint(user1.address, millionUsda);
        await daiContract.withSigner(user1).tx.approve(lendingPool.address, millionUsda);
        await lendingPool.withSigner(user1).tx.deposit(daiContract.address, user1.address, millionUsda, []);
        ...
        await lendingPool.withSigner(user2).tx.deposit(usdcContract.address, user1.address, millionUsdc, []);

        await daiContract.tx.mint(user2.address, millionUsda);
        await daiContract.withSigner(user2).tx.approve(lendingPool.address, millionUsda);
        await lendingPool.withSigner(user2).tx.deposit(daiContract.address, user1.address, millionUsda, []);
      });
      describe('Then User1 and User2 set USDC an DAI as collateral and borrows.\n User1 Borrows 1 million USDC, User2 Borrows 1 million USDax', () => {
        beforeEach('', async () => {
          await lendingPool.withSigner(user1).tx.setAsCollateral(usdcContract.address, true);
          await lendingPool.withSigner(user1).tx.borrow(usdcContract.address, user1.address, millionUsdc, [0]);
          await lendingPool.withSigner(user2).tx.setAsCollateral(usdcContract.address, true);
          await lendingPool.withSigner(user2).tx.borrow(usdaxContract.address, user2.address, millionUsdax, [0]);
        });
