...
describe.skip('Checked Math', () => {
  ...
  it('Trigger overflow by borrow action', async () => {
    ...
    await expect(testEnv.lendingPool.withSigner(user2).query.borrow(reserve.address, user2.address, amountToBorrow, [0])).to.eventually.be.rejected;
  });
  it('Trigger overflow by time travel', async () => {
    ...
    const user = testEnv.users[0];
    const user2 = testEnv.users[1];
    const amountToDeposit = U128_MAX_VALUE.div(new BN(E12.toString()));
