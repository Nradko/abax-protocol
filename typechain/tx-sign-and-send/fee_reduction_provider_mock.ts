...
export default class FeeReductionProviderMockMethods {
   ...
   */
  setFlashLoanFeeReduction(accountId: ArgumentTypes.AccountId | null, feeReduction: number | string | BN, __options?: ContractOptions) {
    return txSignAndSend(
      this.__apiPromise,
      this.__nativeContract,
      this.__keyringPair,
   ...
   */
  getFeeReductions(account: ArgumentTypes.AccountId, __options?: ContractOptions) {
    return txSignAndSend(
      this.__apiPromise,
      this.__nativeContract,
      this.__keyringPair,
      'feeReduction::getFeeReductions',
      (events: EventRecord[]) => {
        return decodeEvents(events, this.__nativeContract, EVENT_DATA_TYPE_DESCRIPTIONS);
      },
      [account],
   ...
   * @param { ArgumentTypes.AccountId } account,
   */
  getFlashLoanFeeReduction(account: ArgumentTypes.AccountId, __options?: ContractOptions) {
    return txSignAndSend(
      this.__apiPromise,
      this.__nativeContract,
      this.__keyringPair,
      'feeReduction::getFlashLoanFeeReduction',
      (events: EventRecord[]) => {
        return decodeEvents(events, this.__nativeContract, EVENT_DATA_TYPE_DESCRIPTIONS);
      },
      [account],
