...
import ContractAbi from '../artifacts/psp22_emitable.json';
import QueryMethods from '../query/psp22_emitable';
import BuildExtrinsicMethods from '../build-extrinsic/psp22_emitable';
import TxSignAndSendMethods from '../tx-sign-and-send/psp22_emitable';
import MixedMethods from '../mixed-methods/psp22_emitable';
import EventsClass from '../events/psp22_emitable';
import EVENT_DATA_TYPE_DESCRIPTIONS from '../event-data/psp22_emitable.json';
import type { EventDataTypeDescriptions } from "../shared/types";


export default class Psp22EmitableContract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new Psp22EmitableContract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : Psp22EmitableContract {
		return new Psp22EmitableContract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : Psp22EmitableContract {
		return new Psp22EmitableContract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : Psp22EmitableContract {
		return new Psp22EmitableContract(this.address, this.signer, api);
	}
}