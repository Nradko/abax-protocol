...
import ContractAbi from '../artifacts/fee_reduction_provider_mock.json';
import QueryMethods from '../query/fee_reduction_provider_mock';
import BuildExtrinsicMethods from '../build-extrinsic/fee_reduction_provider_mock';
import TxSignAndSendMethods from '../tx-sign-and-send/fee_reduction_provider_mock';
import MixedMethods from '../mixed-methods/fee_reduction_provider_mock';
import EventsClass from '../events/fee_reduction_provider_mock';
...
export default class FeeReductionProviderMockContract {
   ...
   * @example
   * ```typescript
   * const contract = new FeeReductionProviderMockContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): FeeReductionProviderMockContract {
    return new FeeReductionProviderMockContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): FeeReductionProviderMockContract {
    return new FeeReductionProviderMockContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): FeeReductionProviderMockContract {
    return new FeeReductionProviderMockContract(this.address, this.signer, api);
  }
}
