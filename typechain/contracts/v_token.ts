...
import ContractAbi from '../artifacts/v_token.json';
import QueryMethods from '../query/v_token';
import BuildExtrinsicMethods from '../build-extrinsic/v_token';
import TxSignAndSendMethods from '../tx-sign-and-send/v_token';
import MixedMethods from '../mixed-methods/v_token';
import EventsClass from '../events/v_token';
import EVENT_DATA_TYPE_DESCRIPTIONS from '../event-data/v_token.json';
import type { EventDataTypeDescriptions } from '../shared/types';

export default class VTokenContract {
   ...
   * @example
   * ```typescript
   * const contract = new VTokenContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): VTokenContract {
    return new VTokenContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): VTokenContract {
    return new VTokenContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): VTokenContract {
    return new VTokenContract(this.address, this.signer, api);
  }
}
