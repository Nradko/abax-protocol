...
import ContractAbi from '../artifacts/psp22_for_audit.json';
import QueryMethods from '../query/psp22_for_audit';
import BuildExtrinsicMethods from '../build-extrinsic/psp22_for_audit';
import TxSignAndSendMethods from '../tx-sign-and-send/psp22_for_audit';
import MixedMethods from '../mixed-methods/psp22_for_audit';
import EventsClass from '../events/psp22_for_audit';
...
export default class Psp22ForAuditContract {
   ...
   * @example
   * ```typescript
   * const contract = new Psp22ForAuditContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): Psp22ForAuditContract {
    return new Psp22ForAuditContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): Psp22ForAuditContract {
    return new Psp22ForAuditContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): Psp22ForAuditContract {
    return new Psp22ForAuditContract(this.address, this.signer, api);
  }
}
