...
import ContractAbi from '../artifacts/test_reserves_minter.json';
import QueryMethods from '../query/test_reserves_minter';
import BuildExtrinsicMethods from '../build-extrinsic/test_reserves_minter';
import TxSignAndSendMethods from '../tx-sign-and-send/test_reserves_minter';
import MixedMethods from '../mixed-methods/test_reserves_minter';
import EventsClass from '../events/test_reserves_minter';
...
export default class TestReservesMinterContract {
   ...
   * @example
   * ```typescript
   * const contract = new TestReservesMinterContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): TestReservesMinterContract {
    return new TestReservesMinterContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): TestReservesMinterContract {
    return new TestReservesMinterContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): TestReservesMinterContract {
    return new TestReservesMinterContract(this.address, this.signer, api);
  }
}
