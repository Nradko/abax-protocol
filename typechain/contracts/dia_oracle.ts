...
import ContractAbi from '../artifacts/dia_oracle.json';
import QueryMethods from '../query/dia_oracle';
import BuildExtrinsicMethods from '../build-extrinsic/dia_oracle';
import TxSignAndSendMethods from '../tx-sign-and-send/dia_oracle';
import MixedMethods from '../mixed-methods/dia_oracle';
import EventsClass from '../events/dia_oracle';
...
export default class DiaOracleContract {
   ...
   * @example
   * ```typescript
   * const contract = new DiaOracleContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): DiaOracleContract {
    return new DiaOracleContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): DiaOracleContract {
    return new DiaOracleContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): DiaOracleContract {
    return new DiaOracleContract(this.address, this.signer, api);
  }
}
