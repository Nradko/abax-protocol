...
import ContractAbi from '../artifacts/test_psp22.json';
import QueryMethods from '../query/test_psp22';
import BuildExtrinsicMethods from '../build-extrinsic/test_psp22';
import TxSignAndSendMethods from '../tx-sign-and-send/test_psp22';
import MixedMethods from '../mixed-methods/test_psp22';
import EventsClass from '../events/test_psp22';
...
export default class TestPsp22Contract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new TestPsp22Contract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : TestPsp22Contract {
		return new TestPsp22Contract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : TestPsp22Contract {
		return new TestPsp22Contract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : TestPsp22Contract {
		return new TestPsp22Contract(this.address, this.signer, api);
	}
}