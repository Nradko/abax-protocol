...
import ContractAbi from '../artifacts/stable_token.json';
import QueryMethods from '../query/stable_token';
import BuildExtrinsicMethods from '../build-extrinsic/stable_token';
import TxSignAndSendMethods from '../tx-sign-and-send/stable_token';
import MixedMethods from '../mixed-methods/stable_token';
import EventsClass from '../events/stable_token';
...
export default class StableTokenContract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new StableTokenContract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : StableTokenContract {
		return new StableTokenContract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : StableTokenContract {
		return new StableTokenContract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : StableTokenContract {
		return new StableTokenContract(this.address, this.signer, api);
	}
}