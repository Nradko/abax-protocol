...
import ContractAbi from '../artifacts/psp22_ownable.json';
import QueryMethods from '../query/psp22_ownable';
import BuildExtrinsicMethods from '../build-extrinsic/psp22_ownable';
import TxSignAndSendMethods from '../tx-sign-and-send/psp22_ownable';
import MixedMethods from '../mixed-methods/psp22_ownable';
import EventsClass from '../events/psp22_ownable';
...
export default class Psp22OwnableContract {
   ...
   * @example
   * ```typescript
   * const contract = new Psp22OwnableContract(address, signerAlice, api);
   * await contract.mint(signerBob.address, 100);
   * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
   * ```
   */
  withSigner(signer: KeyringPair): Psp22OwnableContract {
    return new Psp22OwnableContract(this.address, signer, this.nativeAPI);
  }
   ...
   */
  withAddress(address: string): Psp22OwnableContract {
    return new Psp22OwnableContract(address, this.signer, this.nativeAPI);
  }
   ...
   */
  withAPI(api: ApiPromise): Psp22OwnableContract {
    return new Psp22OwnableContract(this.address, this.signer, api);
  }
}
