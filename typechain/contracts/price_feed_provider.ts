...
import ContractAbi from '../artifacts/price_feed_provider.json';
import QueryMethods from '../query/price_feed_provider';
import BuildExtrinsicMethods from '../build-extrinsic/price_feed_provider';
import TxSignAndSendMethods from '../tx-sign-and-send/price_feed_provider';
import MixedMethods from '../mixed-methods/price_feed_provider';
import EventsClass from '../events/price_feed_provider';
...
export default class PriceFeedProviderContract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new PriceFeedProviderContract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : PriceFeedProviderContract {
		return new PriceFeedProviderContract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : PriceFeedProviderContract {
		return new PriceFeedProviderContract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : PriceFeedProviderContract {
		return new PriceFeedProviderContract(this.address, this.signer, api);
	}
}