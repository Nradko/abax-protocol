...
import StableToken from '../contracts/stable_token';
import { getContractObjectWrapper } from '../shared/utils';
import type BN from 'bn.js';
import FsAPI from 'fs';
import PathAPI from 'path';

const fileName = 'stable_token';

export default class StableTokenDeployer {
   ...
   */
  async new(name: string, symbol: string, decimal: number | string | BN, __options?: ContractOptions) {
    ...
    const contractObj = getContractObjectWrapper(this.nativeAPI, StableToken, address, this.signer);
    return {
