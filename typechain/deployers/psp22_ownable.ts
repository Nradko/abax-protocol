...
import Psp22Ownable from '../contracts/psp22_ownable';
import { getContractObjectWrapper } from '../shared/utils';
import type BN from 'bn.js';
import FsAPI from 'fs';
import PathAPI from 'path';

const fileName = 'psp22_ownable';

export default class Psp22OwnableDeployer {
   ...
   */
  async new(name: string, symbol: string, decimal: number | string | BN, owner: ArgumentTypes.AccountId, __options?: ContractOptions) {
    ...
    const tx = codePromise.tx['new']!({ gasLimit, storageDepositLimit, value: __options?.value }, name, symbol, decimal, owner);
    let response;
    ...
    const contractObj = getContractObjectWrapper(this.nativeAPI, Psp22Ownable, address, this.signer);
    return {
