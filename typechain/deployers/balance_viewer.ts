...
import BalanceViewer from '../contracts/balance_viewer';
import { getContractObjectWrapper } from '../shared/utils';
import type BN from 'bn.js';
import FsAPI from 'fs';
import PathAPI from 'path';

const fileName = 'balance_viewer';

export default class BalanceViewerDeployer {
   ...
   */
  async new(lendingPool: ArgumentTypes.AccountId, __options?: ContractOptions) {
    ...
    const tx = codePromise.tx['new']!({ gasLimit, storageDepositLimit, value: __options?.value }, lendingPool);
    let response;
    ...
    const contractObj = getContractObjectWrapper(this.nativeAPI, BalanceViewer, address, this.signer);
    return {
