...
export default class Psp22ForAuditMethods {
  ...
  tApprove(
    ...
    return queryOkJSON(this.__apiPromise, this.__nativeContract, this.__callerAddress, 'tApprove', [from, to, value], __options, (result) => {
      return handleReturnType(result, getTypeDescription(37, DATA_TYPE_DESCRIPTIONS));
    });
  }
  ...
  totalSupply(__options?: ContractOptions): Promise<QueryReturnType<Result<BN, ReturnTypes.LangError>>> {
    return queryOkJSON(this.__apiPromise, this.__nativeContract, this.__callerAddress, 'psp22::totalSupply', [], __options, (result) => {
      return handleReturnType(result, getTypeDescription(40, DATA_TYPE_DESCRIPTIONS));
    });
  }
