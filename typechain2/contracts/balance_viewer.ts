...
import ContractAbi from '../artifacts/balance_viewer.json';
import QueryMethods from '../query/balance_viewer';
import BuildExtrinsicMethods from '../build-extrinsic/balance_viewer';
import TxSignAndSendMethods from '../tx-sign-and-send/balance_viewer';
import MixedMethods from '../mixed-methods/balance_viewer';
import EventsClass from '../events/balance_viewer';
...
export default class BalanceViewerContract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new BalanceViewerContract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : BalanceViewerContract {
		return new BalanceViewerContract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : BalanceViewerContract {
		return new BalanceViewerContract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : BalanceViewerContract {
		return new BalanceViewerContract(this.address, this.signer, api);
	}
}