...
import ContractAbi from '../artifacts/flash_loan_receiver_mock.json';
import QueryMethods from '../query/flash_loan_receiver_mock';
import BuildExtrinsicMethods from '../build-extrinsic/flash_loan_receiver_mock';
import TxSignAndSendMethods from '../tx-sign-and-send/flash_loan_receiver_mock';
import MixedMethods from '../mixed-methods/flash_loan_receiver_mock';
import EventsClass from '../events/flash_loan_receiver_mock';
...
export default class FlashLoanReceiverMockContract {
	...
	*/
	 ...
	 * @example
	 * ```typescript
	 * const contract = new FlashLoanReceiverMockContract(address, signerAlice, api);
	 * await contract.mint(signerBob.address, 100);
	 * await contract.withSigner(signerBob).transfer(signerAlice.address, 100);
	 * ```
	*/
	withSigner(signer : KeyringPair) : FlashLoanReceiverMockContract {
		return new FlashLoanReceiverMockContract(this.address, signer, this.nativeAPI);
	}
	...
	*/
	withAddress(address : string) : FlashLoanReceiverMockContract {
		return new FlashLoanReceiverMockContract(address, this.signer, this.nativeAPI);
	}
	...
	*/
	withAPI(api : ApiPromise) : FlashLoanReceiverMockContract {
		return new FlashLoanReceiverMockContract(this.address, this.signer, api);
	}
}